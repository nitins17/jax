...
def get_bazel_path(bazel_path_flag):
  for bazel_path in guess_bazel_paths(bazel_path_flag):
    if bazel_path and verify_bazel_version(bazel_path):
      logger.info("Found a compatible Bazel installation.")
      return bazel_path
  logger.info("Unable not find a compatible Bazel installation.")
  return download_and_verify_bazelisk()

def verify_bazel_version(bazel_path):
  """ Verifies if the version of Bazel is compatible with JAX's required
      Bazel version.
  """
...
def download_and_verify_bazelisk():
  ...
  logger.info("Downloading Bazelisk...")
  ...
  # Verify checksum
  logger.info("Verifying the checksum...")
